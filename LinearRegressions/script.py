...
def mapNonLinear(x,p):
    ...
    if p < 0:
        print("p is less than zero");
        return None;
    elif p == 0:
        # print("p is zero. Returning a single column vector");
        # y = x/x;
        y = np.divide(x,x);
        # print("shape of x/x is ",y.shape);
        return y;
    else :
        Xd = x/x;
        for i in range(1, p+1):
            Xd = np.hstack((Xd, x.__pow__(i)))
        return Xd
...
#print("P2: weights learnt for training data : ", w);
#print("P2: error learnt for training data : ", mle);

w_i = learnOLERegression(X_i,y)
mle_i = testOLERegression(w_i,X_i,y)

#print("P2: weights learnt for test data : ", w_i);
#print("P2: error learnt for test data : ", mle_i);
...
#print("P3: weights learnt for training data : ");

for lambd in lambdas:
    w_l = learnRidgeRegression(X_i,y,lambd)
    mses3_train[i] = testOLERegression(w_l,X_i,y)
    mses3[i] = testOLERegression(w_l,Xtest_i,ytest)
    #print("lamba = ", lambd," | MSE_train = ",mses3_train[i], " | MSE_test = ",mses3[i], " | w_l = ", w_l);
    i = i + 1
fig = plt.figure(figsize=[12,6])
plt.subplot(1, 2, 1)
